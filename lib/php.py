...
def get_call_from_el(func_el):
    ...
    :rtype: Call|None
    """
    token = func_el['expr']['name']['parts'][0]
...
def make_calls(body_el):
    ...
    calls = []
    for el in body_el:
        if el['nodeType'] == 'Stmt_Expression' and el['expr']['nodeType'] == 'Expr_FuncCall':
            calls.append(get_call_from_el(el))
    return calls
...
#         call = get_call_from_el(assignment_el[2])
...
class PHP(BaseLanguage):
    ...
    def get_tree(filename, lang_params):
        ...
        cmd = ["php", "lib/get_ast.php", filename]
    ...
    def separate_namespaces(tree):
        ...
        body = []
        for el in tree:
            if el['nodeType'] in ('Stmt_Function',):
                nodes.append(el)
    ...
    def make_nodes(tree, parent):
        ...
        """
        print(tree)
        assert tree['nodeType'] == 'Stmt_Function'
    ...
    def make_class_group(tree, parent):
        ...
        # subgroup_trees, node_trees, body_trees = PHP.separate_namespaces(tree_body)
