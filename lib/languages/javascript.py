...
class Group(engine.Group):
    ...
    def __init__(self, isFunction=True, isAnon=False, **kwargs):
        ...
        while openBracket != -1:
            ...
            * if we managed to create a group, see below
            ...
            if closeBracket == -1:
                logging.info("Could not find closing bracket for open bracket on line %d in file %s" % (self.source.getLineNumber(openBracket), self.name))
    ...
    def generateImplicitNode(self, blocksToRemove):
        ...
        return Node(name=name, source=source, definitionString=self.definitionString,
                    parent=self, lineNumber=self.lineNumber, isFileRoot=isFileRoot)  # isImplicit=True
    ...
    def newGroupFromBlock(self, openBracket, closeBracket):
        ...
        for pattern in self.PATTERNS:
            newGroup = self.newGroupFromSourcesAndPattern(preBlockSource,
                                                          blockSource,
                                                          pattern)
            if newGroup:
                return newGroup
        ...
        # TODO2021
        logging.info("====================")
        logging.info(preBlockSource.sourceString[-100:])
